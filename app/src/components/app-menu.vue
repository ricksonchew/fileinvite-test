    ...
    <ul id="side-menu" class="metismenu">
        <li v-for="item in menuItems" :key="`item-${item.name}`" class="side-nav-title side-nav-item">
            ...
            <router-link v-if="!hasItems(item)" tag="a" :to="`${item.path}`" class="side-nav-link side-nav-link-ref">
                <feather v-if="item.icon" :type="item.icon"></feather>
                <span>{{ item.name }}</span>
                <span v-if="item.badge" :class="'badge badge-' + item.badge.variant + ' float-right font-size-11'">{{ item.badge.text }}</span>
            </router-link>

            <ul v-if="hasItems(item)" class="nav-second-level">
                <li v-for="subitem in item.children" :key="`sub-item-${subitem.name}`" :class="{ 'side-nav-item': hasItems(subitem) }">
                    <a v-if="hasItems(subitem)" href="javascript:void(0);" class="side-nav-link-a-ref">
                        {{ subitem.name }}
                        <span class="menu-arrow"></span>
                    </a>

                    <router-link v-if="!hasItems(subitem)" tag="a" :to="`${item.path}/${subitem.path}`" class="side-nav-link-ref">{{ subitem.name }}</router-link>

                    <ul v-if="hasItems(subitem)" class="nav-third-level">
                        <li v-for="subSubitem in subitem.children" :key="`sub-sub-item-${subSubitem.name}`">
                            <router-link tag="a" :to="`${item.path}/${subitem.path}/${subSubitem.path}`" class="side-nav-link-ref">{{ subSubitem.name }}</router-link>
                        </li>
                    </ul>
                </li>
            </ul>
